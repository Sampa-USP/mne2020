#!/bin/bash
# input paramters
xyzfile="bulk_water.xyz"      # <- arquivo da posições do sistema de interesse (água bulk)
files=("water.top")           # <- nomes dos arquivos topologicos das moléculas serem usados
types=${#files[@]}
molecules=("267")            # <- número de moléculas, compativel com arquivo xyzfile 
lammpsfile="bulk_water.top"   # <- arquivo de saida lammps
inputfile="input.dat"            
coeffsfile="water_coeffs.dat" # <- coeficentes do campo de força a serem usados
# box dimensions
xlo="0.00"                     
xhi="20.00"
ylo="0.00"
