#!/bin/bash
# input paramters
xyzfile="bulk_heptane.xyz"      # <- arquivo da posições do sistema de interesse (água bulk)
files=("heptane.top")           # <- nomes dos arquivos topologicos das moléculas serem usados
types=${#files[@]}
molecules=("38")            # <- número de moléculas, compativel com arquivo xyzfile 
lammpsfile="bulk_heptane.top"   # <- arquivo de saida lammps
inputfile="input.dat"            
coeffsfile="heptane_coeffs.dat" # <- coeficentes do campo de força a serem usados
# box dimensions
xlo="0.00"                     
xhi="21.00"
ylo="0.00"
